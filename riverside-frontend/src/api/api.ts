...
export async function fetchAllSessions() {
  const response = await axios.get(
    `http://localhost:8080/api/v1/sessions/get-all-sessions`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
export async function createSession(sessionName: string) {
    const response = await axios.post(
      `http://localhost:8080/api/v1/sessions/create-session`,
      { name:sessionName },
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
export async function joinSession(sessionCode:string|null){
    ...
    const response = await axios.post(`http://localhost:8080/api/v1/sessions/join-session/${sessionCode}`,{
            headers:{
                Authorization:`Bearer ${token}`
            }
        });
...
export async function sendFinalCallToEndOfRecording(roomName:string,userType:string,sessionId:string){
